  ...
  return db.class.list().then(() => {
    ...
    const processItem = (_val) => {
      if (typeof _val === 'function') {
        // Allow for a callback if a function is passed as an item.
      ...
      if (val[REF]) {
        if (DEBUG) {
          console.info(`Saving ref ${val[REF]}: `, val)
        }
        refs[val[REF]] = val
        delete val[REF]
      }
      ...
      if (val[TYPE] === VERTEX) {
        ...
        if (upsert) {
            ...
            .then((data) => {
              ...
              if (data.length === 1) {
                ...
                if (Object.keys(update).length) {
                  if (DEBUG) {
                    console.info('Updating vertex', JSON.stringify(update))
                  }
                  ...
                  if (DEBUG) {
                    console.info('nothing left to update on vertex')
                  }
                ...
                // insert
                if (DEBUG) {
                  console.info('Creating new vertex')
                }
          ...
          if (DEBUG) {
            console.info('Creating vertex blindly')
          }
          return db.create(VERTEX, myClass || 'V')
            .set(val)
            .one()
        ...
        if (upsert) {
          ...
          return db.select()
            ...
            .then((data) => {
              ...
              if (data.length === 1) {
                ...
                if (Object.keys(update).length) {
                  if (DEBUG) {
                    console.info('Updating edge', JSON.stringify(update), Object.keys(update))
                  }
                  return db.update(data[0][RID])
                    .set(update)
                    .return('AFTER')
                  ...
                  if (DEBUG) {
                    console.info('Nothing left to update on edge')
                  }
                  return Promise.resolve()
                }
              } else {
                if (DEBUG) {
                  console.info('Creating new edge')
                }
                // insert
          ...
          if (DEBUG) {
            console.info('Creating edge blindly')
          }
          // insert
          return db.create(EDGE, myClass || 'V')
            .set(val)
